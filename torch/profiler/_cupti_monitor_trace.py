...
def merge_trace_window_into_chrome_trace(
    ...
    *,
    ...
    tail_insert = len(events)
    for idx, event in enumerate(events):
        if event.get("cat") == "Trace" or event.get("name") in {
            "Iteration Start: PyTorch Profiler",
            "Record Window End",
        }:
            tail_insert = idx
            break
    events[tail_insert:tail_insert] = trace_events
    ...
    for event in events:
        if event.get("cat") == "Trace" and event.get("name") == "PyTorch Profiler (0)":
            event["dur"] = max(max_end_ts - float(event["ts"]), 0.0)
        elif event.get("name") == "Record Window End":
            event["ts"] = max_end_ts + 0.001
